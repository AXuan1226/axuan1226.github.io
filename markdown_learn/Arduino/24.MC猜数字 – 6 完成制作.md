...
> – 如何通过串口监视器查看程序运行状况
> – 如何通过analogRead产生真正的随机数字
> – 完成猜数字装置

<iframe src="//player.bilibili.com/player.html?aid=52628485&bvid=BV164411J7GE&cid=92100248&page=25" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>

请留意，在本视频讲解21：35秒左右，进行程序修改时，我们将变量重新定义在loop函数里面。这将导致该变量是loop函数里面的局部变量。因此该变量在每一次loop函数执行时都会被重置，因此在后续的视频演示中，您所看到数码管显示的数字是8而不是真正的随机数字

应该将变量myMumber重新定义在setup函数前面，即将mynumber定义为全局变量。这样最后的led数码管才能正常显示随机数字。
